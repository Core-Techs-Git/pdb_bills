...
export class SearchOptionsDTOValidator extends Validator {
  public validate(data: SearchOptionsDTO): void {
    ...
    if (data.priceFrom && !v.isInt(data.priceFrom)) throw new ValidationError(`Invalid priceFrom option – ${data.priceFrom}`);
    if (data.priceTo && !v.isInt(data.priceTo)) throw new ValidationError(`Invalid priceTo option – ${data.priceTo}`);
